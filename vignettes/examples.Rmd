...
cities of `r unique(labelled::to_factor(py_temperature$city))`.
...
plot <- py_temperature |> ggplot(aes(x = week, y = avg_temp)) +
  geom_line() +
  facet_wrap(~ labelled::to_factor(city))
ggsave("../vignettes/fig/temp-plot.png", width = 8)
```

```{r, echo=FALSE}
knitr::include_graphics("../man/figures/temp-plot.png")
```
...
which should be the same, otherwise we would have seen an `error` message.
...
see additional meta data which were not available before.